...
void setup() {
  ...
  Serial.println("Enter flicker frequency in Hz (e.g. 40):");
...
// ---- Read user frequency from Serial ----
  if (Serial.available() > 0) {
    ...
    if (frequency > 0 && frequency <= 100) {   // safety limits
      halfPeriod = (unsigned long)(500000.0 / frequency);
      Serial.print("Frequency set to ");
      Serial.print(frequency);
      Serial.println(" Hz");
    } else {
      Serial.println("Invalid frequency (use 1–100 Hz)");
    }
  ...
  if (switchValue == LOW) {
    ...
    while ((endTime - startTime) < halfPeriod) {
